...
if [[ -n "$SSH_CLIENT"  ||  -n "$SSH2_CLIENT" ]]; then
    eval host='$bold$white%M$remote_session_postfix$reset'     # aye, SSH
else
    eval host='$bold$white%M$local_session_postfix$reset'      # nay, local
fi

# Ruby Ruby Ruby Ruby!
local rvm_ruby=''
if which rvm-prompt &> /dev/null; then
    rvm_ruby='$ruby_red$env_prefix$rvm $(rvm-prompt i v p g s)$env_suffix$reset '
else
...
local pre_prompt_lower='$bold$light_yellow $second_line_prefix$reset'
local return_code='%(?..%{$red%}%? $return_line_prefix$reset)'
local user_host='$bold$user$bold$white@$reset$host'
local current_dir='$bold$cyan${PWD/#$HOME/~}$reset'
